			...
			<div class="card-body">
				...
				<tbody role="rowgroup" class=""><!---->
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr role="row" class="">
						<td aria-colindex="1" data-label="Message ID" role="cell" class=""><div><a href="<?php echo U('index/message',array('id'=>$vo['ids']));?>"><?php echo ($vo["message_id"]); ?></div></td>
						<td aria-colindex="2" data-label="Transaction ID" role="cell" class=""><div><a class="address-tag" href="<?php echo U('index/transaction',array('data'=>$vo['transaction_id']));?>"><?php echo ($vo["transaction_id"]); ?></a></div></td>
